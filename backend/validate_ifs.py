...
def ensure_working_startingpointtable():
    """
    Ensure desktop/input/StartingPointTable.xlsx exists. If missing, clone template.
    """
    ...
    if not template.exists():
        raise FileNotFoundError(
            "Missing template: desktop/input/template/StartingPointTable_clean.xlsx"
        )

    working.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy(template, working)
    print("[BIGPOPA] Created working StartingPointTable.xlsx from clean template.")
